...
::: info Pro 深度增强
ProTree 不是简单的 ElTree 封装，而是面向业务场景的深度增强：

- **内置搜索** — 自带搜索框 + 可配防抖（默认 300ms），无需额外代码
- **一键展开/收起** — 暴露 `expandAll()` / `collapseAll()` 方法，支持编程式控制
- **子节点计数** — 自动为每个父节点显示子节点数量徽标
- **自定义过滤** — 支持 `filterMethod` 替换默认的 label 匹配逻辑
- **完整暴露** — 通过 `getTreeRef()` 获取底层 ElTree 实例，保留全部原生能力
  :::
